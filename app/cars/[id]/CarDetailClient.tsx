...
import { type Car, type FeatureSection, formatINR, getCarBrandedName, carFaqData } from "@/lib/data";
import { getCarDekhoImage } from "@/lib/cardekho-image-map";
import { get360Config } from "@/lib/car360Config";

// Both are only mounted once the user opens the 360° viewer or the test
// drive modal — code-splitting them keeps that JS out of every car page's
// initial bundle.
const Car360Viewer = dynamic(() => import("@/components/Car360Viewer"), { ssr: false });
const TestDriveForm = dynamic(() => import("@/components/TestDriveForm"), { ssr: false });
    ...
    <>
      ...
      <main className="mt-[80px] bg-white">
        ...
        <section className="relative overflow-hidden border-b border-border bg-white">
          <div className="container-px mx-auto max-w-[1180px] pt-7 lg:pt-10">
            <nav aria-label="Breadcrumb" className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted">
              ...
              <span className="mx-2 text-faint">/</span>
              <span className="text-text">{getCarBrandedName(car)}</span>
            </nav>
            ...
            <div className="grid grid-cols-1 gap-8 pb-10 pt-7 lg:min-h-[580px] lg:grid-cols-2 lg:gap-12 lg:py-12">
              ...
              <Reveal variant="slide-right" className="flex flex-col gap-6 w-full">
                ...
                {is360Active && car360Config ? (
                  ...
                  <div className="relative flex aspect-[16/10] w-full items-center justify-center overflow-hidden rounded-2xl bg-white border border-slate-200/60 shadow-sm">
                    ...
                    {/* Fixed inset (not a percentage height, which resolves
                        to 0 on a flex child) gives the car visible breathing
                        room on every side instead of touching/overflowing
                        the card edges. The Cyberster's photos are natively
                        very wide/low (convertible roadster) and letterbox
                        under object-contain, so it fills the box edge to
                        edge with object-cover instead - other models keep
                        the original inset/contain behaviour untouched. */}
                    <div className={car.id === "cyberster" ? "absolute inset-0" : "absolute inset-8 sm:inset-12"}>
                      <Image
                        src={activeImage}
                        ...
                        className={`transition-all duration-[600ms] ${car.id === "cyberster" ? "object-cover" : "object-contain"}`}
              ...
              <Reveal variant="slide-left" className="flex flex-col w-full">
                ...
                {/* CTAs - Book a Test Drive and Get a Variant Quote sit side by
                    side as an aligned pair on every breakpoint; Download
                    Brochure joins the row once there's room (sm+), otherwise
                    it sits on its own row below. */}
                <div ref={ctasRef} className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                  <div className="flex gap-3 sm:contents">
                    <button type="button" onClick={() => setShowTestDriveModal(true)} className="group flex flex-1 items-center justify-center gap-2 rounded bg-brand px-6 py-3.5 text-sm font-semibold text-white transition-all hover:bg-brand-light cursor-pointer sm:flex-none sm:inline-flex">
                      Book a Test Drive
                    ...
                    <Link href="/contact-us" className="flex flex-1 items-center justify-center gap-2 rounded border border-brand bg-white px-6 py-3.5 text-sm font-semibold text-brand transition-all hover:bg-brand hover:text-white cursor-pointer sm:flex-none sm:inline-flex">
                      Get a Variant Quote
                    </Link>
                  </div>
                  {car.brochureUrl && (
                    <a href={car.brochureUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 rounded border border-slate-200 bg-slate-50 px-6 py-3.5 text-sm font-semibold text-text transition-all hover:border-slate-400 hover:bg-slate-100 cursor-pointer sm:inline-flex">
        ...
        {/* ── STICKY NAV ── */}
        <nav aria-label="Vehicle sections" className="sticky top-[72px] z-30 block border-b border-slate-200 bg-white/95 backdrop-blur-md shadow-sm">
          <div className="container-px mx-auto flex max-w-[1180px] items-center gap-5 overflow-x-auto py-3 [-ms-overflow-style:none] [scrollbar-width:none] lg:justify-between lg:gap-4 lg:overflow-visible lg:py-3.5 [&::-webkit-scrollbar]:hidden">
            {/* Left / Middle Navigation Links */}
            <div className="flex shrink-0 items-center gap-5 text-[11px] font-bold uppercase tracking-[0.14em] text-slate-600 lg:gap-7">
            ...
            {/* Extreme Right: CTA Buttons - always visible and part of the
                same horizontal scroll on mobile so they never disappear;
                on desktop they still fade in once scrolled past the hero's
                own CTAs, same as before. */}
            <div
              className={`flex shrink-0 items-center gap-2.5 opacity-100 pointer-events-auto transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] lg:translate-y-0 lg:scale-100 ${
                showStickyCtas
                  ? "lg:opacity-100 lg:pointer-events-auto"
                  : "lg:opacity-0 lg:translate-y-1.5 lg:scale-[0.98] lg:pointer-events-none"
              }`}
            >
              <button
                type="button"
            ...
            <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
              <Reveal>
                ...
                <p className="mt-4 max-w-md text-sm leading-7 text-muted">
                  Built to deliver a powerful, connected, and extremely comfortable driving experience. Explore the standard specifications and performance of the {getCarBrandedName(car)}.
                </p>
              </Reveal>
          ...
          <section id="explore" className="scroll-mt-36 bg-[#0a0c10] py-14 text-white lg:py-20">
            <div className="container-px mx-auto max-w-[1180px]">
              <Reveal>
                ...
                <h2 className="mt-2 font-display text-3xl font-black lg:text-4xl">{getCarBrandedName(car)} - Feature Explorer</h2>
              </Reveal>
              ...
              <div className="mt-8 grid items-start gap-6 lg:grid-cols-[320px_1fr]">
                {/* Left: item list */}
                <div className="flex flex-row gap-2 overflow-x-auto lg:flex-col lg:gap-0 lg:self-start">
                  {fs.items.map((item, i) => (
                    <button
                      key={item.title}
                      type="button"
                ...
                {car.detailedSections.map((section, index) => (
                  ...
                  >
                    <div className="relative aspect-[16/10] overflow-hidden rounded-2xl border border-border shadow-[0_16px_40px_-20px_rgba(15,23,42,0.25)]">
                      <Image src={section.image} alt={section.title} title={section.title} fill className="object-cover transition duration-700 hover:scale-105" />
                    </div>
                    <div>
          ...
          <section id="gallery" className="scroll-mt-36 bg-white py-14 lg:py-20">
            <div className="container-px mx-auto max-w-[1180px]">
              <Reveal>
                ...
                <h2 className="mt-2 font-display text-3xl font-black text-text">{getCarBrandedName(car)} gallery</h2>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/92 backdrop-blur-sm" onClick={closeLightbox}>
            ...
            <div className="relative mx-14 max-w-[90vw] max-h-[85vh] overflow-hidden rounded-xl sm:mx-16 sm:max-w-[85vw]" onClick={(e) => e.stopPropagation()}>
              <div className="relative w-full" style={{ maxWidth: 1000, aspectRatio: "16/9" }}>
                <Image src={car.galleryImages[lightboxIndex].src} alt={car.galleryImages[lightboxIndex].caption} title={car.galleryImages[lightboxIndex].caption} fill className="object-contain" sizes="90vw" />
              </div>
        ...
        <section id="enquire" className="scroll-mt-36 bg-brand-deep py-14 text-white lg:py-16">
          <div className="container-px mx-auto flex max-w-[1180px] flex-col justify-between gap-7 md:flex-row md:items-center">
            <Reveal>
              ...
              <h2 className="mt-2 font-display text-3xl font-black">Meet the {getCarBrandedName(car)}</h2>
        ...
        {/* ── FAQ ── */}
        <FAQ id="car-faq" data={carFaqData[car.id] ?? []} />
      </main>
      <Footer />

...
import { cars, getCarBrandedName, type Car, type CarCategory } from "@/lib/data";
    ...
    <>
      ...
      <main className="mt-[80px] min-h-screen bg-bg-2">
        ...
        <section className="py-12 lg:py-16">
          <div className="container-px mx-auto max-w-[1400px]">
            {/* Category selection */}
            <Reveal className="flex gap-1 overflow-x-auto border-b border-border sm:gap-2">
              {categories.map((cat) => (
                <button
                  key={cat}
                    ...
                    >
                       <article className="group flex h-full flex-col overflow-hidden rounded-xl border border-border bg-white shadow-md transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg">
                        ...
                        >
                          ...
                          />
                          {car.category === "Select" && (
                            <span className="absolute left-3 top-3 rounded bg-black/80 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-white backdrop-blur">
                              MG Select
                            </span>
                          )}
                        </Link>
                        ...
                        <div className="flex flex-1 flex-col p-6">
                          ...
                          <div className="mt-auto pt-5">
                            <Link
                              href={`/cars/${car.id}`}
                              aria-label={`Explore details for MG ${car.name}`}
